...
func (a *Actions) Search(ctx context.Context, container string, packageName string) (*SearchResponse, error) {
	var osInfo sandbox.ContainerInfo
	var err error
	if len(container) > 0 {
		osInfo, err = a.validateContainer(ctx, container)
		if err != nil {
...
func (a *Actions) List(ctx context.Context, params ListParams) (*ListResponse, error) {
	...
	builder := sandbox.PackageQueryBuilder{
...
func (a *Actions) GetFilterFields(_ context.Context) (GetFilterFieldsResponse, error) {
	return sandbox.DistroFilterConfig.FieldsInfo(), nil
}
...
func (a *Actions) validateContainer(ctx context.Context, container string, autoUpdate ...bool) (sandbox.ContainerInfo, error) {
	shouldAutoUpdate := len(autoUpdate) == 0 || autoUpdate[0]
	container = strings.TrimSpace(container)
	if container == "" {
		selected, err := a.selectContainerInteractive(ctx)
		if err != nil {
			return sandbox.ContainerInfo{}, err
		}
		container = selected
	}
	...
	if errInfo != nil {
		if err := a.serviceDistroDatabase.ContainerDatabaseExist(ctx, container); err == nil {
			if err = a.serviceDistroDatabase.DeletePackagesFromContainer(ctx, container); err != nil {
				return sandbox.ContainerInfo{}, apmerr.New(apmerr.ErrorTypeDatabase, fmt.Errorf(app.T_("Failed to delete container records: %w"), err))
			}
		}

		return sandbox.ContainerInfo{}, apmerr.New(apmerr.ErrorTypeNotFound, errInfo)
	}
	...
	if shouldAutoUpdate {
		if err := a.serviceDistroDatabase.ContainerDatabaseExist(ctx, container); err != nil {
			if _, err = a.servicePackage.UpdatePackages(ctx, osInfo); err != nil {
				return sandbox.ContainerInfo{}, apmerr.New(apmerr.ErrorTypeDatabase, err)
			}
		}
	}
