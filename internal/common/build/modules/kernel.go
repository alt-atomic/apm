...
func (b *KernelBody) Execute(ctx context.Context, bc pkgbuild.RuntimeContext) (any, error) {
	return withDomain(ctx, bc, b.run)
}

func (b *KernelBody) run(ctx context.Context, svc DomainContext) (any, error) {
...
func currentKernelInfo(ctx context.Context, svc DomainContext) (*service.Info, error) {
	svc.EmitEvent(ctx, reply.StateBefore, reply.EventKernelCurrent, "")
	defer svc.EmitEvent(ctx, reply.StateAfter, reply.EventKernelCurrent, "")
...
func init() { pkgbuild.Register(TypeKernel, func() pkgbuild.Body { return &KernelBody{} }) }
