...
`JobStarted` приходит, когда задача действительно вышла из очереди и начала
выполняться. Итог приходит в `JobFinished`; его также можно получить через
`Jobs.Get`, поэтому пропущенный сигнал не приводит к потере результата.

Фоновые задачи `Packages`, `Kernel`, `Applications.Update` и `Repo.TestTask`
используют общий ресурс `host` и выполняются последовательно в порядке
создания. Задачи `Image` занимают отдельный ресурс `image`: сборка и
переключение образа не трогают пакетную базу хоста и идут параллельно
с пакетными операциями. В сессионном сервисе операции Distrobox выполняются
последовательно для одного контейнера; операции с разными контейнерами могут
идти параллельно. Читающая фоновая задача `Repo.TaskPackages` не занимает ресурс
...
Поле `state` в ответах `Jobs.Get` и `Jobs.List` имеет значение `queued`, пока
задача ожидает ресурс, `running` во время выполнения, затем `ok`, `error` или
`canceled`. Завершённая задача остаётся в реестре ещё 20 минут: `Jobs.Get`
отдаёт её итоговое состояние и результат, если клиент пропустил сигнал. После
этого `Jobs.Get` вернёт `NotFound`.

Отмена: владелец задачи отменяет свободно, чужой — через polkit-действие того
домена, которому задача принадлежит. Ожидающая отменяемая задача удаляется из
очереди и не запускается. Задачи с `cancellable: false`, включая пакетные
транзакции, отменять нельзя: `Jobs.Cancel` вернёт ошибку валидации.
